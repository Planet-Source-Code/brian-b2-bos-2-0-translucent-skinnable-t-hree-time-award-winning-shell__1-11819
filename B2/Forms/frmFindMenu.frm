...
Sub LoadSkinImages()
    For i = 0 To 3
        picItem(i).Picture = LoadPicture(GetSkinImage("Start Menu\Find Menu\Normal\Image" & i & ".bmp"))
        picItemOver(i).Picture = LoadPicture(GetSkinImage("Start Menu\Find Menu\Over\Image" & i & ".bmp"))
    Next
End Sub
...
Private Sub picItem_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    Select Case Index
        Case 0
            ShellFile "http://www.msn.com"
        Case 1
            ShellFile "http://www.altavista.com"
        Case 2
            ShellFile "http://www.planetsourcecode.com/vb/scripts/search.asp?lngWid=1"
        Case 3
            On Error GoTo Err:
            picItem(3).LinkTopic = "Folders|AppProperties"
            picItem(3).LinkMode = vbLinkManual
            picItem(3).LinkExecute "[FindFolder(""C:\"", ""C:\"")]"
            Exit Sub
Err:
            Err.Clear
            MsgBoxEX "Explorer is not running and therefore the Find File dilog box will not work. There may be a fix to this problem in a later release of B2 ", "Explorer not running - B2 Error 2", sndError
    End Select
End Sub
